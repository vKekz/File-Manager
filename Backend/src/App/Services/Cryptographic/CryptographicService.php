...
readonly class CryptographicService implements CryptographicServiceInterface
{
    ...
    function encrypt(string $data): string
    {
        ...
        $hash = $this->generateHash($encrypted, true);
    ...
    function decrypt(string $input): string | false
    {
        ...
        $recreatedHash = $this->generateHash($encrypted, true);
     ...
     * @inheritdoc
     */
    function generateHash(string $data, bool $binary = false): string
    {
        return hash_hmac(self::HASH_ALGORITHM, $data, $this->environment->get(EnvironmentKey::HASH_KEY), $binary);
    }
}